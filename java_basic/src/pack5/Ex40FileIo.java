...
//1 바이트 단위의 데이터에 대한 파일 입출력 - 보통 클래스의 이름에 streamdl 포함됨
...
public class Ex40FileIo {
	public void writeFile(File file, ArrayList<String> list) {
		try {
			...
			for(String ss:list) {
				write.write(ss, 0, ss.length());
				write.newLine();//다음 행으로 이돈 (line skip)
			}
			write.close();
		} catch (Exception e) {
			e.printStackTrace();
	...
	public void readFile(File file) {
		try {
			...
			e.printStackTrace();
			System.out.println("readFilie err : " + e.getMessage());
		}
	}
