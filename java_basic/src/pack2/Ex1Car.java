...
//믈래스 형식 : [접근 지정자] class  클래스명 
public class Ex1Car {	//Car 객체 (Object)를 생산하기 위한 설계도 :클래스	
	//새로운 타입(type, 형)을 만들기 위함
	//field(속성)과 method(행위) 멤버로 가질 수 있음
	//동일한 field와 method를 수행하는 객체의 집합을 설계
	
	int wheel;	// 멤버 필드 형식은 [접근지정자] type 필드명 int wheel = 0; 없으면 접근지정자는 default
	private int airBag = 1;	//캡슐화 : 멤버 접근지정자가 private 인 경우
	private int speed;
	public String irum;
